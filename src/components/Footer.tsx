...
import { footerService, type Footer } from "../services/footerService";
...
const Footer = () => {
  ...
  useEffect(() => {
    const fetchFooter = async () => {
      try {
        setLoading(true);
        const data = await footerService.getActive();
        setFooter(data);
      } catch (error) {
      ...
      <footer className="bg-deep-green text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center py-8">
            <div className="text-light-green">Loading footer...</div>
          </div>
        </div>
      </footer>
    );
    ...
    <footer className="bg-deep-green text-white py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8 mb-12">
          ...
          {/* Research Areas */}
          {footer.research_areas_links && footer.research_areas_links.length > 0 && (
            <div>
              <h4 className="text-lg font-heading font-bold mb-4">
                Research Areas
              </h4>
              <ul className="space-y-2 text-light-green">
                {footer.research_areas_links.map((link, index) => (
                  <li key={index}>
                    {link.url.startsWith('/') ? (
                      <Link
                        to={link.url}
          ...
          {/* Publications */}
          {footer.publications_links && footer.publications_links.length > 0 && (
            <div>
              <h4 className="text-lg font-heading font-bold mb-4">
                Publications
              </h4>
              <ul className="space-y-2 text-light-green">
                {footer.publications_links.map((link, index) => (
                  <li key={index}>
                    {link.url.startsWith('/') ? (
                      <Link
                        to={link.url}
